...
class Usuarios extends CI_Controller
{
    ...
    public function core($usuario_id = null)
    {
        ...
        if (!$usuario_id) {
            ...
            $this->form_validation->set_rules('username', 'Username', 'trim|required|min_length[4]|max_length[50]|callback_valida_usuario');
            ...
            if (!$usuario = $this->ion_auth->user($usuario_id)->row()) {
                ...
                if ($this->form_validation->run()) {
                    ...
                    $password = $this->input->post('password');
                    /*
                    * Nao atualiza a senha se a mesma não for passada
                    */
    ...
    public function delete($usuario_id = NULL){
        ...
        if(!$usuario_id  || !$this->ion_auth->user($usuario_id)->row()){
            ...
            $this->ion_auth->set_flashdata('erro', 'O usuario nao foi encontrado');
            redirect('restrita/usuarios');
        }else{
            //começa a exclusão
            if($this->ion_auth->is_admin($usuario_id)){
                $this->session->set_flashdata('erro','Não é permitido excluir um usuario com perfil de Administrador');
                redirect('restrita/usuarios');
            }
